...
date: 2026-03-05
categories: [data engineering,docker,airflow, orchestration,airflow v3.0.0]
...
write = DockerOperator(
    ...
    mounts=[Mount(source='./data', target='/tmp/data', type='bind')],
    dag=dag
)
```

So simply, this task is designed just to run an `Alpine` container that writes a file to a mounted volume. The `data` sub-directory in the current directory (project directory) is mounted to the `/tmp/data` address of the container.

![The task it scheduled successfully in the DAG seen in the Airflow UI](../assets/img/09-airflow-test-write-00.png)
...
#Dynamically get the absolute path to the data directory
...
#Obtain the host path from the environment variable
